	...
	for i = 1, #_G["Layout"] do
		...
		if i == selected then
			if v["Type"] == "Category" then
				...
				v["Drawings"]["Text"].Color = _G["Theme"]["Selected_Color"]
				v["Drawings"]["Text"].Size = _G["Theme"]["Text_Size"]
				if v["Drawings"]["Text"].Text:sub(1, 1) ~= ">" then
					v["Drawings"]["Text"].Text = ">" .. v["Drawings"]["Text"].Text
				end
				if
					v["Type"] == "Toggle"
				...
				then
					...
					if not v["Drawings"]["Extra"]["Text"].Text:sub(1, 1) ~= ">" then
			...
			if v["Type"] == "Category" then
				...
				then
					...
					v["Drawings"]["Extra"]["Text"].Size = _G["Theme"]["Text_Size"]
					if v["Drawings"]["Extra"]["Text"].Text:sub(1, 1) == ">" then
			...
			for i = 1, n do
				...
				if i == selected then
					...
					elseif v["Type"] == "Slider" then
						...
						local newval = clamp(
							round((v["VALUE"] + v["Increment"]) * tonumber(str)) / tonumber(str),
							v["Min"],
							v["Max"]
						)
						v["VALUE"] = newval
...
function Library:NewCategory(cat_name, color)
	...
	_G["Layout"][val] = {
		["Type"] = "Category",
		["Drawings"] = CreateTextBox(
			cat_name,
			RGB(10, 10, 10),
			0.75,
			color or _G["Theme"]["Category_Text"],
			_G["Theme"]["Text_Size"],
			v2(_G["Theme"]["UI_Position"].X, new_y),
			{ Type = "Category" }
		),
	}
	Library:Reset()
	...
	function cat_funcs:NewButton(op_name, CallBack)
		...
		_G["Layout"][val] = {
			...
			["Drawings"] = CreateTextBox(
				...
				RGB(255, 255, 255),
				_G["Theme"]["Text_Size"],
				v2(_G["Theme"]["UI_Position"].X + 10, new_y),
				{ Type = "Button" }
			),
			["CallBack"] = CallBack,
		}
	...
	function cat_funcs:NewColorpicker(op_name, default, CallBack)
		...
		_G["Layout"][val] = {
			...
			["Drawings"] = CreateTextBox(
				...
				_G["Theme"]["Text_Size"],
				v2(_G["Theme"]["UI_Position"].X + 10, new_y),
				{ Type = "Colorpicker", Color = default }
			),
			["Selected"] = 1,
			["CallBack"] = CallBack,
		}
